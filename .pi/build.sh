...
# docker buildx build --tag venus --file .pi/Dockerfile --platform linux/arm64 --cache-from venus:cache --cache-to venus:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar venus
...
sudo bash -c "echo venus > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/venus.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/venus.img
